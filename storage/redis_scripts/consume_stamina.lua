-- consume_stamina_continuous_optimized.lua
-- Versão analítica contínua otimizada para reduzir processamento
...
local function safe_encode(tbl)
  ...
  for k,v in pairs(tbl) do
    if not first then table.insert(parts,",") end first=false
...
local function main()
  local hkey, field = KEYS[1], ARGV[1]
  ...
  -- cálculo regeneração contínua otimizada
  ...
  -- consumo ou recuperação
  if amount < 0 then
    local new_current = math.min(sMax, current - amount)
    ...
    return safe_encode({used=used,recovered=-amount,current_after=new_current,note="recovered"})
  end
